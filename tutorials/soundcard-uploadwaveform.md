...
The `Harp.SoundCard` Bonsai package supports preload and dynamic update of sound waveforms during runtime. The following exercises will demonstrate how to generate and load various waveforms for upload to the `SoundCard` in Bonsai.
...
- Insert a [`FunctionGenerator`] source and configure the following properties:
   - `Amplitude` - Set this to the target volume in 32-bit range.
   - `BufferLength` - Set this to the target sample number, which will determine the sound length  in combination with the `SampleRate` property (e.g. 192000 for a 2 s sound at 96 kHz sample rate).
   ...
   - `SampleRate` - Set this to the target sample rate, in Hz (e.g. 96000).
...
- Insert a [`CreateDiscreteUniform`] source and set the `Lower` and `Upper` properties to the target volume in 32-bit range.
